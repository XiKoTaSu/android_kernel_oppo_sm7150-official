...
static const struct apsd_result *smblib_get_apsd_result(struct smb_charger *chg)
{
	...
	if (g_oplus_chip->charger_type ==  POWER_SUPPLY_TYPE_USB_DCP && result->pst == POWER_SUPPLY_TYPE_USB && chg->hvdcp_disable == false) {
		chg_err("keep dcp!!  result->pst=%d   chg->hvdcp_disable=%d\n",result->pst,chg->hvdcp_disable);
...
#ifdef VENDOR_EDIT
  	smblib_err(chg, "usbtemp test suspend=%d, usb_status=%d\n", suspend, chg->usb_status);
	if(chg->usb_status)
		suspend = true;
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
	if ((opluschg_is_usb_present() == false) && (first_boot == true)) {
		if (opluschg_get_otg_switch() == false && !opluschg_get_otg_online()) {
			opluschg_set_otg_switch(false);
			first_boot = false;
			pr_info("%s: First Boot done,set typec snk_only\n", __func__);
		}
	}
#endif
...
/* Yichun.Chen	PSW.BSP.CHG  2019-05-08  for otg switch */
	if(opluschg_is_usb_present()) {
		chg_err("boot with usb_online not set snk_only\n");
	} else {
		rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
...
static void opluschg_monitor_func(struct work_struct *work)
{
	...
	if (counts >= (chip->batt_full ? 8 : 3)) {//because rechg counts=6
		...
		if (stat & PRE_TERM_BIT) {
			...
			msleep(50);
			#ifdef VENDOR_EDIT
			smblib_err(chg, "usbtemp test usb_status=%d\n", chg->usb_status);
			if(!chg->usb_status)
	...
	while (!kthread_should_stop()) {
		if (opluschg_usbtemp_condition() == true) {
			...
			if (get_project() == 19111) {
				check_temperature_list2(temperature_list1, total_time, critical_rise1, index);
				check_temperature_list2(temperature_list2, total_time, critical_rise2, index);
			} else {
			...
			if (get_project() == 19111) {
				check_dischg_enable2(temperature1, temperature2, critical_rise1, critical_rise2, bat_temp, dischg_enable);
			} else {
